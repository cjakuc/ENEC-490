...
for i = window/2 + 1:num_hours-window/2
    ...
    window_mean = mean(transformed_data(i-250):transformed_data(i+250));
    
    % calculate the std. deviation for every point in transformed data from
    % (i-250) to (i + 250) (a 501 point window)
    window_std = std(transformed_data(i-250):transformed_data(i+250));
...
% Conversion of vector of hourly demand to a vector of peak electricity
% demand
vector_peak = hourly_peak(vector);
